...
def test_create_organisation():
    with TestClient(app) as client:
        clean_database()
        response = client.post("/admin/organisations", json=new_organisation_data)
...
def test_add_and_remove_member():
    with TestClient(app) as client:
        clean_database()
        organisation_response = client.post("/admin/organisations", json=new_organisation_data)
        ...
        response = client.post("/admin/organisations/add_user", json=member_json)
        assert response.status_code == 200

        response = client.post("/admin/organisations/remove_user", json=member_json)
        assert response.status_code == 200
