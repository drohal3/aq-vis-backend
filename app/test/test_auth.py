...
from src.database.operations.auth import create_login_access_token, get_current_user, get_auth_user
...
def test_access_token():
    with TestClient(app):
        ...
        user = get_auth_user(database=database, email=new_user_data["email"], password=new_user_data["password"])

        assert user["email"] == new_user_data["email"]

        access_token = create_login_access_token(database=database, email=new_user_data["email"])
